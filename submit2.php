	...
	<body>
		...
		<div class="center" id="wordcloud">
			<?php
				...
				// add current artist's lyrics to lyrics so far, adds other info to the current session
				$newarray = add_to_cloud($artist);

				//var_dump($newarray);

				//$newarray = $_SESSION['allWords'];
				
				$filtered = filter_stopwords($newarray, $stopwords);
